...
class AddSickCreditCommand extends ContainerAwareCommand {

    protected function configure() {
        parent::configure();

        $this
                ->setName('gbs:leave:addsickcredit')
                ->setDescription('Add yearly sick credit depending of the country')
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        $users = $em->getRepository('GbsLeaveBundle:User')
           ...
           ->andWhere('c.sickCreditPerYear <> 0')
           ->getQuery()->getResult();

        foreach($users as $user) {
            if(!in_array($user->getUsername(), $excluded)) {
                $credit = new EmployeeLeave();
                $credit->setType(10); // sick credit
                $credit->setState(2); // accepted
                $credit->setDays($user->getCountry()->getSickCreditPerYear());
                $credit->setUser($user);
                $credit->setComment("Automatic sick credit");
                $em->persist($credit);
            }
        }
        $em->flush();
